...
class PatchDiscriminator(nn.Module):
    def __init__(self, in_ch=3, hid=64):
        super().__init__()
        self.net = nn.Sequential(
...
def save_recon_grid(model, dl, epoch, device, N=3, out_dir="recon_vis_vqgan"):
    model.eval()
    ...
    fig.suptitle(
        f'VQGAN Epoch {epoch} | Recon: {F.mse_loss(x_r, x).item():.4f} | '
...
def train():
    ...
    print(f"VQ-GAN | Flowers-102 @ 64x64 | codebook: {num_codes} x {embed_dim}d")
    print(f"Device: {device} | disc_start: {disc_start} | disc_w: {disc_weight} | perc_w: {perc_weight}")
    ...
    model = VQGAN(embed_dim=embed_dim, hidden_dim=hidden_dim, num_codes=num_codes).to(device)
    disc = PatchDiscriminator().to(device)
    perc_loss_fn = PerceptualLoss().to(device)

    opt_g = torch.optim.Adam(model.parameters(), lr=lr, betas=(0.5, 0.999))
    opt_d = torch.optim.Adam(disc.parameters(), lr=disc_lr, betas=(0.5, 0.999))
    ...
    for epoch in range(num_epochs):
        model.train()
        disc.train()
        tot_r = tot_vq = tot_g = tot_d = tot_p = 0.0
        n = 0
        ...
        for x, _ in train_loader:
            ...
            p_loss = perc_loss_fn(x_recon, x)

            if use_disc:
                fake_logits = disc(x_recon)
                g_loss = -fake_logits.mean()
                gen_loss = recon_loss + vq_loss + perc_weight * p_loss + disc_weight * g_loss
            else:
                g_loss = torch.tensor(0.0)
                gen_loss = recon_loss + vq_loss + perc_weight * p_loss
            ...
            if use_disc:
                ...
                fake_logits = disc(x_recon.detach())
        ...
        print(
            ...
            f"Perc: {tot_p/n:.4f}  G: {tot_g/n:.4f}  D: {tot_d/n:.4f}  Usage: {usage}/{num_codes}"
        )
    ...
    torch.save(model.state_dict(), "vqgan_flowers102.pt")
    torch.save(disc.state_dict(), "vqgan_disc_flowers102.pt")
    print("Saved vqgan_flowers102.pt + vqgan_disc_flowers102.pt")
