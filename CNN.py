...
class Encoder(nn.Module):
    def __init__(self, in_channels=3, hidden_dim=128, num_features=128):
        super().__init__()
...
class BinaryVQVAE(nn.Module):
    def __init__(self, num_features=64, embed_dim=128, hidden_dim=128, in_channels=3):
        super().__init__()
    ...
    def compute_loss(self, x, x_recon, logits, binary, diversity_w=0.01, binarization_w=0.5):
...
def save_recon_grid(model, dl, epoch, device, N=3, out_dir="recon_vis_v4"):
    model.eval()
...
def save_bit_stats(model, dl, epoch, device, num_samples=100, out_dir="bit_stats"):
    ...
    print(f"  Saved bit stats for {len(analysis_classes)} classes -> {out_dir}/")


def train():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    num_features, embed_dim, hidden_dim = 512, 512, 512
    diversity_w = 0
    binarization_w = 0
    ...
    print(f"Binarization weight: {binarization_w}")
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 5 == 0:
            ...
            save_recon_grid(model, test_loader, epoch + 1, device, N=3, out_dir="recon_vis_v4")
            save_bit_stats(model, test_loader, epoch + 1, device, num_samples=100, out_dir="bit_stats")
            print()
