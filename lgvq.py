...
def build_landmark_geodesic(dataset, num_landmarks=1000, k=10, batch_size=128):
    print(f"Building landmark geodesic (L={num_landmarks}, k={k})...")
    ...
    landmarks = _farthest_point_sample(all_feats, num_landmarks)
    print(f"  Selected {len(landmarks)} landmarks via FPS")
    ...
    print(f"  Computing shortest paths from {len(landmarks)} landmarks...")
    geo_dist = shortest_path(graph, method='D', directed=False, indices=landmarks)
    geo_dist = np.nan_to_num(geo_dist, nan=0.0, posinf=0.0)
    max_val = geo_dist.max() if geo_dist.max() > 0 else 1.0
    geo_dist = geo_dist / max_val
    print(f"  Done. Matrix shape: {geo_dist.shape}, mem: {geo_dist.nbytes/1e6:.1f}MB")

    return torch.tensor(geo_dist, dtype=torch.float32), torch.tensor(landmarks, dtype=torch.long)

def _farthest_point_sample(feats, num_points):
    n = len(feats)
    ...
    for _ in range(num_points - 1):
        ...
        d = np.linalg.norm(feats - last, axis=1)
        min_dists = np.minimum(min_dists, d)
        selected.append(np.argmax(min_dists))
    return np.array(selected)

def landmark_isomap_loss(z_e, data_indices, geo_matrix, landmarks, device, max_pairs=512):
    B, D, H, W = z_e.shape
    ...
    L = landmarks.size(0)
    num_pairs = min(max_pairs, n * L)

    lm_sel = torch.randint(0, L, (num_pairs,))
    batch_sel = torch.randint(0, n, (num_pairs,))

    data_global = data_indices[batch_sel]
    geo_d = geo_matrix[lm_sel, data_global].to(device)
    ...
    di = data_indices[idx_i]
    dj = data_indices[idx_j]
    gi = geo_matrix[:, di]
    gj = geo_matrix[:, dj]
    geo_d = (gi + gj).min(dim=0).values.to(device)

    enc_d = torch.norm(z_pool[idx_i.to(device)] - z_pool[idx_j.to(device)], dim=-1)
    enc_d = enc_d / (enc_d.max().detach() + 1e-8)
    geo_d = geo_d / (geo_d.max().detach() + 1e-8)
    return F.mse_loss(enc_d, geo_d)

def denorm(x): return (x * 0.5 + 0.5).clamp(0, 1)
def save_recon_grid(model, dl, epoch, device, N=3, out_dir="recon_vis_landmark"):
    model.eval()
    x, y, _ = next(iter(dl))
    x = x[:N].to(device)
    ...
    fig.suptitle(f'Landmark Ep{epoch} | Recon {F.mse_loss(x_r, x).item():.4f} | Usage {np.mean(usages):.0f}/{model.vq.M}')
    for i in range(N):
...
def train():
    ...
    iso_w, num_landmarks = 0.01, 1000
    print(f"DCVQ+LandmarkIso (EMA) | 16x8d | 512 codes | iso_w={iso_w} | L={num_landmarks} | {device}")
    ...
    geo_matrix, landmarks = build_landmark_geodesic(dataset, num_landmarks=num_landmarks)
    print(f"  Landmark matrix: {geo_matrix.shape}, mem: {geo_matrix.nelement()*4/1e6:.1f}MB")
    ...
    for epoch in range(50):
        ...
        for x, _, idx in loader:
            ...
            iso_loss = landmark_isomap_loss(z_e, idx, geo_matrix, landmarks, device)
    ...
    print("=" * 70)
    torch.save(model.state_dict(), "dcvq_landmark_iso.pt")
    print("Saved dcvq_landmark_iso.pt")
