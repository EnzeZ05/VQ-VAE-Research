...
class LearnProjVQQuantizer(nn.Module):
    def __init__(self, embed_dim=256, num_codes=512, num_subspaces=8, beta=0.25):
        super().__init__()
        ...
        self.Q_raw = nn.Parameter(torch.zeros(self.D, self.D))
        self.codebooks = nn.ParameterList([nn.Parameter(torch.randn(self.M, self.ds) * 0.02) for _ in range(self.N)])

    def _get_Q(self):
        A = self.Q_raw - self.Q_raw.t()
        I = torch.eye(self.D, device=A.device)
        return torch.linalg.solve(I + A, I - A)

    def _nearest(self, x, cb):
        x2 = (x * x).sum(-1, keepdim=True)
        c2 = (cb * cb).sum(-1).unsqueeze(0)
        dist2 = x2 + c2 - 2.0 * (x @ cb.t())
        idx = dist2.argmin(-1)
        return idx, cb[idx]

    def rotation_angle(self):
        Q = self._get_Q()
        I = torch.eye(self.D, device=Q.device)
        return torch.norm(Q - I).item()
...
def save_recon_grid(model, dl, epoch, device, N=3, out_dir="recon_vis_projvq"):
    model.eval()
    x, y = next(iter(dl))
    ...
    fig.suptitle(f'Epoch {epoch} | Recon {F.mse_loss(x_r, x).item():.4f} | Usage {np.mean(usages):.0f}/{model.vq.M}', fontsize=11, fontweight='bold')
    for i in range(N):
...
def train():
    ...
    embed_dim, hidden_dim, num_codes, num_subspaces = 256, 256, 512, 8
    batch_size, num_epochs, lr = 64, 50, 3e-4
    print(f"ProjVQ-VAE | Flowers-102 @ 64x64 | {num_subspaces}x{embed_dim//num_subspaces} | {num_codes} codes | {device}")
    ...
    model = VAE(embed_dim=embed_dim, hidden_dim=hidden_dim, num_codes=num_codes, num_subspaces=num_subspaces).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        rot_angle = model.vq.rotation_angle()

        print(f"Epoch [{epoch+1:3d}/{num_epochs}]  Recon: {tot_r/n:.4f}  VQ: {tot_vq/n:.4f}  Usage: {avg_usage:.0f}/{num_codes}  RotAngle: {rot_angle:.4f}")
    ...
    torch.save(model.state_dict(), "projvq_flowers102.pt")
    print("Saved projvq_flowers102.pt")
