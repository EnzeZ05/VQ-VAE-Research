...
class TreeLevel(nn.Module):
    def __init__(self, embed_dim, num_splits, num_codes, beta=0.25):
        super().__init__()
        self.num_splits = num_splits
        self.ds = embed_dim // num_splits
        self.beta = beta
    ...
    def forward(self, z_flat):
        chunks = z_flat.chunk(self.num_splits, dim=-1)
        ...
        vq = vq / self.num_splits
        return z_q, vq, idx_parts

class HierarchicalVQ(nn.Module):
    def __init__(self, embed_dim=256, num_codes=512, min_ds=8):
        super().__init__()
        self.embed_dim = embed_dim
        self.levels = nn.ModuleList()
        self.level_splits = []
        n = 1
        while embed_dim // n >= min_ds:
            self.levels.append(TreeLevel(embed_dim, n, num_codes))
            self.level_splits.append(n)
            n *= 2
        self.num_levels = len(self.levels)
        self.alpha = nn.Parameter(torch.ones(self.num_levels) / self.num_levels)

    def forward(self, z):
        B, D, H, W = z.shape
        z_flat = z.permute(0, 2, 3, 1).reshape(-1, D)

        z_qs, vq_total, all_indices = [], 0.0, []
        for level in self.levels:
            z_q, vq, idxs = level(z_flat)
            z_qs.append(z_q)
            vq_total += vq
            all_indices.append(idxs)
        vq_total = vq_total / self.num_levels

        weights = F.softmax(self.alpha, dim=0)
        z_merged = sum(w * zq for w, zq in zip(weights, z_qs))

        z_out = z_merged.view(B, H, W, D).permute(0, 3, 1, 2).contiguous()
        return z_out, vq_total, all_indices, weights

class HierVQVAE(nn.Module):
    def __init__(self, embed_dim=256, hidden_dim=256, num_codes=512, min_ds=8):
        super().__init__()
        self.encoder = Encoder(hid=hidden_dim, out_dim=embed_dim)
        self.decoder = Decoder(hid=hidden_dim, in_dim=embed_dim)
        self.vq = HierarchicalVQ(embed_dim, num_codes, min_ds)

    def forward(self, x):
        z_e = self.encoder(x)
        z_q, vq_loss, all_indices, weights = self.vq(z_e)
        x_r = self.decoder(z_q)
        return x_r, vq_loss, all_indices, weights

    def compute_usage(self, all_indices):
        usages = {}
        for l, idxs in enumerate(all_indices):
            splits = self.vq.level_splits[l]
            ds = self.vq.embed_dim // splits
            level_usage = []
            for idx in idxs:
                level_usage.append(idx.unique().numel())
            usages[f"L{l}({splits}x{ds})"] = np.mean(level_usage)
        return usages

def denorm(x): return (x * 0.5 + 0.5).clamp(0, 1)

def save_recon_grid(model, dl, epoch, device, N=3, out_dir="recon_vis_hiervq"):
    model.eval()
    x, y = next(iter(dl))
    x, y = x[:N].to(device), y[:N]
    with torch.no_grad(): x_r, _, all_indices, weights = model(x)
    x_np = denorm(x.cpu()).permute(0, 2, 3, 1).numpy()
    r_np = denorm(x_r.cpu()).permute(0, 2, 3, 1).numpy()
    usages = model.compute_usage(all_indices)
    w_str = " ".join([f"{w:.2f}" for w in weights.cpu().numpy()])
    fig, axes = plt.subplots(3, N, figsize=(2.8*N, 8))
    fig.suptitle(
        f'HierVQ Epoch {epoch} | Recon {F.mse_loss(x_r, x).item():.4f} | weights: [{w_str}]',
...
def train():
    ...
    embed_dim, hidden_dim, num_codes, min_ds = 256, 256, 512, 8
    batch_size, num_epochs, lr = 64, 50, 3e-4

    model = HierVQVAE(embed_dim=embed_dim, hidden_dim=hidden_dim, num_codes=num_codes, min_ds=min_ds).to(device)
    levels_str = " → ".join([f"{s}x{embed_dim//s}" for s in model.vq.level_splits])
    print(f"HierVQ-VAE | Flowers-102 @ 64x64 | {model.vq.num_levels} levels: {levels_str} | {num_codes} codes")
    ...
    for epoch in range(num_epochs):
        ...
        model.eval()
        with torch.no_grad():
            weights = F.softmax(model.vq.alpha, dim=0).cpu().numpy()
            w_str = " ".join([f"{w:.3f}" for w in weights])
            sample_usages = {}
            for x, _ in test_loader:
                x = x.to(device)
                _, _, all_idx, _ = model(x)
                sample_usages = model.compute_usage(all_idx)
                break
        usage_str = " | ".join([f"{k}:{v:.0f}" for k, v in sample_usages.items()])

        print(
            f"Epoch [{epoch+1:3d}/{num_epochs}]  Recon: {tot_r/n:.4f}  VQ: {tot_vq/n:.4f}  "
            f"Weights: [{w_str}]  {usage_str}"
        )
    ...
    weights = F.softmax(model.vq.alpha, dim=0).cpu().numpy()
    for l, (s, w) in enumerate(zip(model.vq.level_splits, weights)):
        print(f"  Level {l}: {s}x{embed_dim//s}d  weight={w:.4f}")
    torch.save(model.state_dict(), "hiervq_flowers102.pt")
    print("Saved hiervq_flowers102.pt")
